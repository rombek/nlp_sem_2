...
from score_translit import TRANSLIT_PATH, load_dataset
...
class PositionalEncoding(nn.Module):
    def __init__(self, hidden_size, max_len=512):
        ...
        pos_arg = np.repeat(np.arange(max_len, dtype=np.float32).reshape(-1, 1), hidden_size, axis=1)
        pos_arg /= 10000.0**(np.arange(hidden_size, dtype=np.float32) / hidden_size)

        pos_arg[:, 1::2] = np.cos(pos_arg[:, ::2])
        pos_arg[:, ::2] = np.sin(pos_arg[:, ::2])
       

        pe = torch.tensor(pos_arg)
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask):
        ...
        attn_weights = (query @ key.transpose(-1, -2)) / np.sqrt(self.head_hidden_size)
        attn_weights = attn_weights * mask
        attn_weights[attn_weights == 0] = -np.inf
...
class LrScheduler:
    def __init__(self, n_steps, **kwargs):
        ...
        if self.type == 'warmup,decay_linear':
            ...
            self.warmup_delta = self.lr_peak / self.warmup_steps
            self.decrease_delta = self.lr_peak / (n_steps - self.warmup_steps)
    ...
    def learning_rate(self, step=None):
        ...
        if self.type == 'warmup,decay_linear':
            ...
            if step <= self.warmup_steps:
                self._lr = step * self.warmup_delta
            else:
                self._lr = self.lr_peak - (self.decrease_delta * (step - self.warmup_steps))
            # raise NotImplementedError
        return self._lr
...
def train(source_strings, target_strings):
    ...
    # prepate dev data
    dev_part = 'dev_small'
    dev_data = load_dataset(TRANSLIT_PATH, parts=(dev_part, ))
    dev_ids, dev_strings, dev_transliterations = dev_data[dev_part]

    dev_source_strings, dev_target_strings = zip(*sorted(zip(dev_strings, dev_transliterations),
                                                 key=lambda e: len(e[0])))

    dev_dataloader = create_dataloader(dev_source_strings, dev_target_strings, text_encoder, train_config['batch_size'], shuffle_batches_each_epoch=False)
    ...
    for epoch in range(1,train_config['n_epochs']+1):
        ...
        # make evaluation on dev
        max_decoding_len = model.config['max_tgt_seq_length']
        model.eval()
        dev_predictions = generate_predictions(dev_dataloader, max_decoding_len, text_encoder, model, device)

        dev_metrics = compute_metrics(dev_predictions, dev_target_strings, ['acc@1'])
        for name, val in dev_metrics.items():
            tqdm.write(f"Dev {name}: {val}")
